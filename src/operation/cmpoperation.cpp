...
Value CmpOperation::generate_value(llvm::IRBuilder<>& builder) {
  ...
  Value result(typesafety::TypeInformation::BOOL_T(builder.getContext()),
               cmps.at(0), builder, lhs.get_line());
  if (cmps.size() > 1) {
    for (auto* cmp : std::vector<llvm::Value*>(cmps.begin() + 1, cmps.end())) {
      result =
          Value(result.type_info, builder.CreateAnd(result.get_llvm_val(), cmp),
                builder, result.get_line());
    }
  }
...
CmpOperation::gather_and_resolve_proposals(llvm::LLVMContext& ctx) {
  ...
  if (typesafety::TypeResolver::try_resolve(proposals, ctx))
